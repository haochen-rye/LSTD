#ifndef CAFFE_SPARSE_MULTIBOX_LOSS_LAYER_HPP_
#define CAFFE_SPARSE_MULTIBOX_LOSS_LAYER_HPP_
...
class SparseMultiBoxLossLayer : public LossLayer<Dtype> {
 public:
  explicit SparseMultiBoxLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "SparseMultiBoxLoss"; }
 ...
 protected:
  ...
  Blob<Dtype> loc_gt_;
  // blbo which stores the corresponding weights
  Blob<Dtype> loc_bbox_weight_;
  // localization loss.
  ...
  Blob<Dtype> conf_gt_;
  // blob which stores the corresponding weights
  Blob<Dtype> conf_weight_;
  // confidence loss.
  ...
  float neg_min_overlap_;
  float neg_max_overlap_;
...
#endif  // CAFFE_SPARSE_MULTIBOX_LOSS_LAYER_HPP_

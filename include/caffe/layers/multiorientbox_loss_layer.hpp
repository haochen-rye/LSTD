#ifndef CAFFE_MULTIORIENTBOX_LOSS_LAYER_HPP_
#define CAFFE_MULTIORIENTBOX_LOSS_LAYER_HPP_
...
class MultiOrientBoxLossLayer : public LossLayer<Dtype> {
 public:
  explicit MultiOrientBoxLossLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "MultiOrientBoxLoss"; }
 ...
 protected:
  ...
  // The internal rotation loss layer
  shared_ptr<Layer<Dtype> > rotation_loss_layer_;
  LocLossType rotation_loss_type_;
  float rotation_weight_;
  // bottom vector holder used in Forward function.
  vector<Blob<Dtype>*> rotation_bottom_vec_;
  // top vector holder used in Forward function.
  vector<Blob<Dtype>*> rotation_top_vec_;
  // blob which stores the matched rotationation prediction.
  Blob<Dtype> rotation_pred_;
  // blob which stores the corresponding matched ground truth.
  Blob<Dtype> rotation_gt_;
  // rotationalization loss.
  Blob<Dtype> rotation_loss_;
  ...
  MultiBoxLossParameter multiorientbox_loss_param_;
  int num_classes_;
...
#endif  // CAFFE_MULTIORIENTBOX_LOSS_LAYER_HPP_

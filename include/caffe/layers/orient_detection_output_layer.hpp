#ifndef CAFFE_ORIENT_DETECTION_OUTPUT_LAYER_HPP_
#define CAFFE_ORIENT_DETECTION_OUTPUT_LAYER_HPP_
...
class OrientDetectionOutputLayer : public Layer<Dtype> {
 public:
  explicit OrientDetectionOutputLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  ...
  virtual inline const char* type() const { return "OrientDetectionOutput"; }
 ...
 protected:
   ...
   *
   * @param bottom input Blob vector (at least )
   *   -# @f$ (N \times C1 \times 1 \times 1) @f$
   *      the location predictions with C1 predictions.
   *   -# @f$ (N \times C1 \times 1 \times 1) @f$
   *      the rotation predictions with C1 predictions.   
  ...
  Blob<Dtype> rotation_preds_;
  Blob<Dtype> rotation_permute_;
  Blob<Dtype> conf_permute_;
};

}  // namespace caffe

#endif  // CAFFE_ORIENT_DETECTION_OUTPUT_LAYER_HPP_

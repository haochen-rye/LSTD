...
__global__ void WeightedSoftmaxCrossEntropyLossForwardGPU(const int nthreads,
...
template <typename Dtype>
void WeightedSoftmaxWithCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  WeightedSoftmaxCrossEntropyLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
...
template <typename Dtype>
__global__ void WeightedSoftmaxCrossEntropyLossBackwardGPU(const int nthreads,
          const Dtype* predict_prob_data, const Dtype* target_prob_data,
...
template <typename Dtype>
void WeightedSoftmaxWithCrossEntropyLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    caffe_gpu_mul(predict_prob_.count(), target_prob_data, bottom_diff, bottom_diff);
...
INSTANTIATE_LAYER_GPU_FUNCS(WeightedSoftmaxWithCrossEntropyLossLayer);

...
#include "caffe/layers/sparse_multibox_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void SparseMultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  loc_bbox_weight_.Reshape(loc_shape);
  loc_bottom_vec_.push_back(&loc_pred_);
  loc_bottom_vec_.push_back(&loc_gt_);
  loc_bottom_vec_.push_back(&loc_bbox_weight_);
  ...
  conf_bottom_vec_.push_back(&conf_weight_);
  ...
  layer_param.set_type("WeightedSoftmaxWithLoss");
  ...
  conf_weight_.Reshape(conf_shape);
...
template <typename Dtype>
void SparseMultiBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SparseMultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  //                  &all_neg_indices_);
  int batch_size = all_loc_preds.size();
  SparseHardExamples(all_match_overlaps, multibox_loss_param_,batch_size,
  ...
  if (num_conf_ >= 1) {
    ...
    Dtype* conf_weight_data = conf_weight_.mutable_cpu_data();
    Dtype* conf_gt_data = conf_gt_.mutable_cpu_data();
    caffe_set(conf_gt_.count(), Dtype(background_label_id_), conf_gt_data);
    caffe_set(conf_weight_.count(), Dtype(0), conf_weight_data);
    EncodeConfPrediction(conf_data, num_, num_priors_, multibox_loss_param_,
                         all_match_indices_, all_neg_indices_, all_gt_bboxes,
                         conf_pred_data, conf_gt_data, conf_weight_data);
    // LOG(INFO) << "the weight sum: " 
    //   << caffe_cpu_asum(conf_weight_.count(), conf_weight_data);
...
template <typename Dtype>
void SparseMultiBoxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_CLASS(SparseMultiBoxLossLayer);
REGISTER_LAYER_CLASS(SparseMultiBoxLoss);

...
#include "caffe/layers/multiorientbox_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void MultiOrientBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const MultiBoxLossParameter& multiorientbox_loss_param =
      this->layer_param_.multibox_loss_param();
  multiorientbox_loss_param_ = this->layer_param_.multibox_loss_param();

  num_ = bottom[0]->num();
  num_priors_ = bottom[2]->height() / 4;
  // Get other parameters.
  CHECK(multiorientbox_loss_param.has_num_classes()) << "Must provide num_classes.";
  num_classes_ = multiorientbox_loss_param.num_classes();
  CHECK_GE(num_classes_, 1) << "num_classes should not be less than 1.";
  share_location_ = multiorientbox_loss_param.share_location();
  loc_classes_ = share_location_ ? 1 : num_classes_;
  background_label_id_ = multiorientbox_loss_param.background_label_id();
  use_difficult_gt_ = multiorientbox_loss_param.use_difficult_gt();
  mining_type_ = multiorientbox_loss_param.mining_type();
  if (multiorientbox_loss_param.has_do_neg_mining()) {
    LOG(WARNING) << "do_neg_mining is deprecated, use mining_type instead.";
    do_neg_mining_ = multiorientbox_loss_param.do_neg_mining();
  ...
  loc_weight_ = multiorientbox_loss_param.loc_weight();
  loc_loss_type_ = multiorientbox_loss_param.loc_loss_type();
  // fake shape.
  ...
  // set up rotation loss layer
  rotation_weight_ = multiorientbox_loss_param.rotation_loss_weight();
  rotation_loss_type_ = multiorientbox_loss_param.rotation_loss_type();
  // fake shape
  vector<int> rotation_shape(1,1);
  rotation_shape.push_back(1);
  rotation_pred_.Reshape(rotation_shape);
  rotation_gt_.Reshape(rotation_shape);
  rotation_bottom_vec_.push_back(&rotation_pred_);
  rotation_bottom_vec_.push_back(&rotation_gt_);
  rotation_loss_.Reshape(loss_shape);
  rotation_top_vec_.push_back(&rotation_loss_);
  if (rotation_loss_type_ == MultiBoxLossParameter_LocLossType_L2) {
    LayerParameter layer_param;
    layer_param.set_name(this->layer_param_.name() + "_l2_rotation");
    layer_param.set_type("EuclideanLoss");
    layer_param.add_loss_weight(rotation_weight_);
    rotation_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
    rotation_loss_layer_->SetUp(rotation_bottom_vec_, rotation_top_vec_);
  } else if (rotation_loss_type_ == MultiBoxLossParameter_LocLossType_SMOOTH_L1) {
    LayerParameter layer_param;
    layer_param.set_name(this->layer_param_.name() + "_smooth_L1_rotation");
    layer_param.set_type("SmoothL1Loss");
    layer_param.add_loss_weight(rotation_weight_);
    rotation_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
    rotation_loss_layer_->SetUp(rotation_bottom_vec_, rotation_top_vec_);
  } else {
    LOG(FATAL) << "Unknown rotationa loss type.";
  }
  // Set up confidence loss layer.
  conf_loss_type_ = multiorientbox_loss_param.conf_loss_type();
...
template <typename Dtype>
void MultiOrientBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(num_priors_ * loc_classes_, bottom[1]->channels());
  CHECK_EQ(num_priors_ * num_classes_, bottom[2]->channels())
      << "Number of priors must match number of confidence predictions.";
}

template <typename Dtype>
void MultiOrientBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const Dtype* loc_data = bottom[0]->cpu_data();
  const Dtype* rotation_data = bottom[1]->cpu_data();
  ...
  GetOrientGroundTruth(gt_data, num_gt_, background_label_id_, use_difficult_gt_,
                 &all_gt_bboxes);
  ...
  FindMatches(all_loc_preds, all_gt_bboxes, prior_bboxes, prior_variances,
              multiorientbox_loss_param_, &all_match_overlaps, &all_match_indices_);

  num_matches_ = 0;
  int num_negs = 0;
  // Sample hard negative (and positive) examples based on mining type.
  MineHardExamples(*bottom[2], all_loc_preds, all_gt_bboxes, prior_bboxes,
                   prior_variances, all_match_overlaps, multiorientbox_loss_param_,
                   &num_matches_, &num_negs, &all_match_indices_,
                   &all_neg_indices_);

  if (num_matches_ >= 1) {
    // Form data to pass on to loc_loss_layer_ and rotation_loss_layer_.
    // Reshape loc_blob
    ...
    // Reshape rotation_blob
    vector<int> rotation_shape(2);
    rotation_shape[0] = 1;
    rotation_shape[1] = num_matches_;
    rotation_pred_.Reshape(rotation_shape);
    rotation_gt_.Reshape(rotation_shape);
    Dtype* rotation_pred_data = rotation_pred_.mutable_cpu_data();
    Dtype* rotation_gt_data = rotation_gt_.mutable_cpu_data();    
    // Encode and initialize the loc/rotation_loss_layer_;
    EncodeLocPrediction(all_loc_preds, rotation_data, all_gt_bboxes,
                        all_match_indices_, prior_bboxes, prior_variances, 
                        multiorientbox_loss_param_, loc_pred_data, loc_gt_data,
                        rotation_pred_data, rotation_gt_data);
    loc_loss_layer_->Reshape(loc_bottom_vec_, loc_top_vec_);
    loc_loss_layer_->Forward(loc_bottom_vec_, loc_top_vec_);

    rotation_loss_layer_->Reshape(rotation_bottom_vec_, rotation_top_vec_);
    rotation_loss_layer_->Forward(rotation_bottom_vec_, rotation_top_vec_);

  } else {
    loc_loss_.mutable_cpu_data()[0] = 0;
    rotation_loss_.mutable_cpu_data()[0] = 0;
  }
  ...
  if (this->layer_param_.propagate_down(1)) {
    Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
        normalization_, num_, num_priors_, num_matches_);
    top[0]->mutable_cpu_data()[0] += 
      rotation_weight_ * rotation_loss_.cpu_data()[0] / normalizer;
  }
...
template <typename Dtype>
void MultiOrientBoxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  // Back propagate on rotation prediction.
  if (propagate_down[1]) {
    Dtype* rotation_bottom_diff = bottom[1]->mutable_cpu_diff();
    caffe_set(bottom[1]->count(), Dtype(0), rotation_bottom_diff);
    if (num_matches_ >= 1) {
      vector<bool> rotation_propagate_down;
      // Only back propagate on prediction, not ground truth.
      rotation_propagate_down.push_back(true);
      rotation_propagate_down.push_back(false);
      rotation_loss_layer_->Backward(rotation_top_vec_, rotation_propagate_down,
                                rotation_bottom_vec_);
      // Scale gradient.
      Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
          normalization_, num_, num_priors_, num_matches_);
      Dtype loss_weight = top[0]->cpu_diff()[0] / normalizer;
      caffe_scal(rotation_pred_.count(), loss_weight, rotation_pred_.mutable_cpu_diff());
      // Copy gradient back to bottom[1].
      const Dtype* rotation_pred_diff = rotation_pred_.cpu_diff();
      int count = 0;
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int> >::iterator it =
             all_match_indices_[i].begin();
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            // Copy the diff to the right place.
            int start_idx = loc_classes_ * j + label;
            caffe_copy<Dtype>(1, rotation_pred_diff + count,
                              rotation_bottom_diff + start_idx);
            ++count;
          }
        }
        rotation_bottom_diff += bottom[1]->offset(1);
      }
    }
  }
...
INSTANTIATE_CLASS(MultiOrientBoxLossLayer);
REGISTER_LAYER_CLASS(MultiOrientBoxLoss);

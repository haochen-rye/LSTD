...
#include "caffe/layers/orient_detection_evaluate_layer.hpp"
#include "caffe/util/bbox_util.hpp"

namespace caffe {

template <typename Dtype>
void OrientDetectionEvaluateLayer<Dtype>::LayerSetUp(
  ...
  evaluate_rotation_ = detection_evaluate_param.evaluate_rotation();
...
template <typename Dtype>
void OrientDetectionEvaluateLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void OrientDetectionEvaluateLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  const Dtype* det_data = bottom[0]->cpu_data();
  ...
  GetOrientDetectionResults(det_data, bottom[0]->height(), background_label_id_,
                      &all_detections);

  // Retrieve all ground truth (including difficult ones).
  map<int, LabelBBox> all_gt_bboxes;
  GetOrientGroundTruth(gt_data, bottom[1]->height(), background_label_id_,
                 true, &all_gt_bboxes);
        ...
        if (label_bboxes.find(label) == label_bboxes.end()) {
          ...
          for (int i = 0; i < bboxes.size(); ++i) {
            ...
            for (int j = 0; j < gt_bboxes.size(); ++j) {
              float overlap = OrientJaccardOverlap(bboxes[i], gt_bboxes[j],
                                   use_normalized_bbox_, evaluate_rotation_);
...
INSTANTIATE_CLASS(OrientDetectionEvaluateLayer);
REGISTER_LAYER_CLASS(OrientDetectionEvaluate);

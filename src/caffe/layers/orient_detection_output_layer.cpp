...
#include "caffe/layers/orient_detection_output_layer.hpp"

namespace caffe {

template <typename Dtype>
void OrientDetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  rotation_preds_.ReshapeLike(*(bottom[0]));
  if (!share_location_) {
    bbox_permute_.ReshapeLike(*(bottom[0]));
    rotation_permute_.ReshapeLike(*(bottom[0]));
  }
  conf_permute_.ReshapeLike(*(bottom[2]));
}

template <typename Dtype>
void OrientDetectionOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(num_priors_ * num_loc_classes_ , bottom[1]->channels())
      << "Number of priors must match number of rotation predictions.";      
  ...
  // [image_id, label, confidence, xmin, ymin, xmax, ymax, rotation]
  top_shape.push_back(8);
  top[0]->Reshape(top_shape);
}

template <typename Dtype>
void OrientDetectionOutputLayer<Dtype>::Forward_cpu(
  ...
  GetLocPredictions(loc_data, rotation_data, num, num_priors_, num_loc_classes_,
  ...
  for (int i = 0; i < num; ++i) {
    ...
    // LOG(INFO) << "Keep_top_k: " << keep_top_k_ << "\tnum_det: " << num_det;
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        top_data[count * 8 + 7] = bbox.rotation();
        ++count;
      }
    }
  }
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(OrientDetectionOutputLayer, Forward);
#endif

INSTANTIATE_CLASS(OrientDetectionOutputLayer);
REGISTER_LAYER_CLASS(OrientDetectionOutput);

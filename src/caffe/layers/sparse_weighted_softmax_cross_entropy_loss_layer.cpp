...
#include "caffe/layers/sparse_weighted_softmax_cross_entropy_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void SparseWeightedSoftmaxWithCrossEntropyLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void SparseWeightedSoftmaxWithCrossEntropyLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ( bottom[2]->count(), outer_num_ * inner_num_);
...
template <typename Dtype>
void SparseWeightedSoftmaxWithCrossEntropyLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SparseWeightedSoftmaxWithCrossEntropyLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    const Dtype* prob_weight_data = bottom[2]->cpu_data();
    int num_classes = predict_prob_.shape(softmax_axis_);    
    for (int i = 0; i < bottom[2]->count(); ++i){
      if (prob_weight_data[i] == 0){
        caffe_scal(num_classes, Dtype(0), bottom_diff + i * num_classes);
      }
    }
...
#ifdef CPU_ONLY
STUB_GPU(SparseWeightedSoftmaxWithCrossEntropyLossLayer);
#endif

INSTANTIATE_CLASS(SparseWeightedSoftmaxWithCrossEntropyLossLayer);
REGISTER_LAYER_CLASS(SparseWeightedSoftmaxWithCrossEntropyLoss);

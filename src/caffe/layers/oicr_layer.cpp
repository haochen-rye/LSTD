...
#include "caffe/layers/oicr_layer.hpp"

namespace caffe {

template <typename Dtype>
void OICRLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void OICRLayer<Dtype>::Reshape(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {

  }

template <typename Dtype>
void OICRLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* bottom_score = bottom[2]->cpu_data();
  ...
  int num_instance = 0;
  GetGroundTruth(bottom_label, bottom_proposal, bottom_score,
    batch_size_,channels_, actual_classes_, num_proposal_,
    max_score_threshold_, use_max_score_threshold_,
...
INSTANTIATE_CLASS(OICRLayer);
REGISTER_LAYER_CLASS(OICR);
